...
import {append_product, patch_product} from '../redux_reducer/productsSlice';
...
function Upload({all_items, append_product, itemId}: props) {
    let info = undefined;
    if(itemId!==undefined){
    info = all_items.filter((item)=>{return item.id === itemId})[0];
   }
    ...
    let theSize = "S";
    let theColor = 'Black';
    // if(info !== undefined && Object.keys(info).includes('size')){
    //     theSize = info.size;
    //     theColor = info.color;
    // }
    ...
    return (
        <>
        {info !== undefined ?  <div className={styles.upload__form__wrapper}>
            ...
            <Formik<formValues>
                initialValues={{
                    ...
                    color: theColor,
                    sign: info.price[0],
                    amount: parseInt(info.price.slice(1)),
                }} 
                enableReinitialize
                onSubmit={async (values: formValues, { resetForm }): Promise<void> => {
                    ...
                    dispatch(patch_product({id: (itemId !== undefined ? itemId : -1), image: getPic(values.picture), 
                    label: values.category, title:values.description, price:`${values.sign}${values.amount}`, 
                    sign:values.sign, amount:values.amount, size:values.size}));
                    ...
                    //     , title:values.description, price:`${values.sign}${values.amount}`, sign:values.sign,
                    // amount:values.amount, size:values.size}));
                }}
                ...
                {props => (
                    <form className={styles.upload__form} autoComplete='off' onSubmit={props.handleSubmit}>
                        ...
                        <input
                            ...
                            // multiple
                            onChange={(e) => {
                                console.log('the value', props.values);
                        ...
                        <br/>
                        {
                            ...
                            value={item}
                            name="category"
                            checked={item === props.values.category}
                        />
                        ...
                        >
                            ...
                            {sizes.map((s) => {
                                return (<option value={s} label={s} selected={s === props.values.size}>
                                       {" "}
                                       {s}
                                       </option>);
                            })}
                        ...
                        >
                            ...
                            {colors.map((c) => {
                                return (<option value={c} label={c} selected={c === props.values.color}>
                                       {" "}
                                       {c}
                                       </option>);
                            })}
                        ...
                        >
                            ...
                            {dollar_signs.map((d) => {
                                return (<option value={d} label={d} selected={d === props.values.sign}>
                                       {" "}
                                       {d}
                                       </option>);
                            })}
